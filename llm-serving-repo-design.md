...
Unlike a single-branch repo, **each branch here carries a different tree** — only its own environment folder and workflow file, not all three side by side. This is the actual layout on the `main` branch:
...
├── .github/
│   └── workflows/
│       └── deploy-prod.yml           # THIS BRANCH ONLY — Dev has deploy-dev.yml, Qa has deploy-qa.yml
│
├── models/
│   ├── gemma-4-31b-it/
...
│       └── config.env
│
├── main/                              # THIS BRANCH ONLY — Dev has dev/, Qa has qa/
...
Dev            ← auto-deploys to DEV environment (3 GPU VMs, one per model)
 ...
 │  PR
 ▼
Qa             ← auto-deploys to QA environment (single VM, all three models)
 │
 │  PR
 ▼
main           ← auto-deploys to PRODUCTION (1× H200 per model, single VM)
```
...
| `Qa` | `qa/` | `deploy-qa.yml` | 2× L40S per model, single VM |
| `main` | `main/` | `deploy-prod.yml` | 1× H200 per model, single VM |
...
## Environment Overrides (main branch, `main/`)

### main/gemma-4-31b-it.env
...
# Same TP (fits on 1 GPU either way), but H200's much larger HBM lets us
# raise context length, batching, and concurrency well past what the L40S
# deployments in dev/qa can safely hold.
...
# Same TP, but H200 headroom lets prod run a much larger context window,
# higher concurrency, and more images per request than the L40S dev/qa tier.
TENSOR_PARALLEL_SIZE=1
DTYPE=bfloat16
MAX_MODEL_LEN=32768
GPU_MEMORY_UTILIZATION=0.90
...
# The biggest divergence in this whole config set: a single H200 has 141GB
# HBM3e, so the ~54GB of BF16 weights fit on ONE GPU with plenty of room
# left for KV cache — tensor parallelism is no longer needed at all.
# Dropping TP 2 -> 1 here isn't just a tuning knob, it changes how many
# GPUs this deployment consumes per replica (dev/qa on 2x L40S need TP=2).
TENSOR_PARALLEL_SIZE=1
DTYPE=bfloat16
MAX_MODEL_LEN=32768
GPU_MEMORY_UTILIZATION=0.90
...
`Dev`'s `dev/*.env` and `Qa`'s `qa/*.env` files are the "small GPU" tier — 2× L40S (48GB each), `TENSOR_PARALLEL_SIZE=2` for the two larger models. `main` is the only environment where a single H200's 141GB HBM lets every model (even `medgemma-27b-it`) run at `TENSOR_PARALLEL_SIZE=1`.
...
./scripts/deploy.sh medgemma-27b-it main
```
...
## Server Mapping — Production Environment

`main` runs **all three models on a single H200 VM** — same single-VM topology as `Qa`, but on H200 hardware with enough HBM that every model runs at `TENSOR_PARALLEL_SIZE=1` (no sharding needed, even for the 27B model):
...
```
          ...
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
       gemma-4-31b-it-main  medgemma-1.5-4b-it-main  medgemma-27b-it-main
       (container)           (container)              (container)
       runner: prod-gpu (same runner/VM for all three deploy jobs)
          │                   │                   │
         vLLM                vLLM                vLLM
          │                   │                   │
        :8000               :8000               :8000
...
Register this VM's runner once:
```bash
./config.sh --url https://github.com/Nitinakrys/Demo_LLM-Serving-Repository-Design --token <TOKEN> --labels self-hosted,prod-gpu
```

Only `Dev` needs per-model runner labels (`dev-4b`/`dev-gemma`/`dev-27b`), since it's the only environment split across multiple physical machines — `Qa` and `main` each use one runner label for all three deploy jobs.

---

## CI/CD Workflow — .github/workflows/deploy-prod.yml (actual, current)

```yaml
name: Deploy to PROD

# main branch -> deployment/prod/*.env (H200). Note the branch is called
# "main", not "prod" — the environment/folder is still named "prod" to
# match what's actually running there; this workflow is what makes that
# main -> prod mapping explicit instead of implicit tribal knowledge.
on:
  push:
    branches: [main]
...
jobs:
  ...
  deploy-gemma-4-31b-it:
    ...
    environment: production    # requires approval — configure in repo Settings > Environments
    steps:
      - uses: actions/checkout@v4
      - name: Deploy gemma-4-31b-it to PROD
        run: ./scripts/deploy.sh gemma-4-31b-it main

  # Job id can't contain dots (GitHub Actions requirement) — the model
  # identifier itself (medgemma-1.5-4b-it, matching models/ and main/ paths)
  ...
  deploy-medgemma-1-5-4b-it:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Deploy medgemma-1.5-4b-it to PROD
        run: ./scripts/deploy.sh medgemma-1.5-4b-it main
  ...
  deploy-medgemma-27b-it:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Deploy medgemma-27b-it to PROD
        run: ./scripts/deploy.sh medgemma-27b-it main
```

`deploy-dev.yml` and `deploy-qa.yml` follow the identical pattern — only the branch trigger (`Dev`/`Qa`), runner label(s), `environment:` name (`dev`/`qa`, neither requiring approval), and the `deploy.sh` env-folder argument (`dev`/`qa`) differ. `Dev`'s version additionally targets a different runner label per job (`dev-4b`/`dev-gemma`/`dev-27b`) since it's split across three VMs.
...
7. **Production requires manual approval.** `deploy-prod.yml`'s jobs set `environment: production` — if that environment is configured under GitHub **Settings → Environments** with required reviewers, every prod deploy pauses for a human sign-off before running.
