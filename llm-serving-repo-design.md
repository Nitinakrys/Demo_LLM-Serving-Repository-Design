...
Unlike a single-branch repo, **each branch here carries a different tree** — only its own environment folder and workflow file, not all three side by side. This is the actual layout on the `Qa` branch:
...
├── .github/
│   └── workflows/
│       └── deploy-qa.yml             # THIS BRANCH ONLY — Dev has deploy-dev.yml, main has deploy-prod.yml
│
├── models/
│   ├── gemma-4-31b-it/
...
│       └── config.env
│
├── qa/                                # THIS BRANCH ONLY — Dev has dev/, main has main/
...
## Environment Overrides (Qa branch, `qa/`)

### qa/gemma-4-31b-it.env
...
These are identical to `Dev`'s `dev/*.env` files except `ENVIRONMENT=qa` and `LOG_LEVEL=INFO` (vs `dev`/`DEBUG`) — same GPU tuning, since `Qa` runs on the same L40S hardware profile as `Dev`. `main`'s `main/*.env` files run on a single H200 each — `TENSOR_PARALLEL_SIZE=1` even for `medgemma-27b-it` (fits on one H200's 141GB), with much higher `MAX_MODEL_LEN`/`MAX_NUM_SEQS` since there's more headroom.
...
## Server Mapping — QA Environment

Unlike `Dev` (split across 3 VMs, one per model), `Qa` runs **all three models on a single VM** — same 2× L40S GPU profile as `Dev`, just one shared box instead of three:
...
```
          ...
          ┌───────────────────┼───────────────────┐
          ▼                   ▼                   ▼
       gemma-4-31b-it-qa   medgemma-1.5-4b-it-qa   medgemma-27b-it-qa
       (container)          (container)             (container)
       runner: qa-gpu (same runner/VM for all three deploy jobs)
          │                   │                   │
         vLLM                vLLM                vLLM
          │                   │                   │
        :8000               :8000               :8000
...
```bash
./config.sh --url https://github.com/Nitinakrys/Demo_LLM-Serving-Repository-Design --token <TOKEN> --labels self-hosted,qa-gpu
```
...
## CI/CD Workflow — .github/workflows/deploy-qa.yml (actual, current)
...
jobs:
  ...
  deploy-gemma-4-31b-it:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Deploy gemma-4-31b-it to QA
        run: ./scripts/deploy.sh gemma-4-31b-it qa

  # Job id can't contain dots (GitHub Actions requirement) — the model
  # identifier itself (medgemma-1.5-4b-it, matching models/ and qa/ paths)
  ...
  deploy-medgemma-1-5-4b-it:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Deploy medgemma-1.5-4b-it to QA
        run: ./scripts/deploy.sh medgemma-1.5-4b-it qa
  ...
  deploy-medgemma-27b-it:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Deploy medgemma-27b-it to QA
        run: ./scripts/deploy.sh medgemma-27b-it qa
```

`deploy-dev.yml` and `deploy-prod.yml` follow the identical pattern — only the branch trigger (`Dev`/`main`), runner label(s), `environment:` name (`dev`/`production`), and the `deploy.sh` env-folder argument (`dev`/`main`) differ. `Dev`'s version additionally targets a different runner label per job (`dev-4b`/`dev-gemma`/`dev-27b`) since it's split across three VMs. `deploy-prod.yml`'s jobs set `environment: production`, which requires manual approval if configured under **Settings → Environments** in GitHub.
