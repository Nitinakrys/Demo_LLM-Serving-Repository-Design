...
main/<model>.env               # THIS BRANCH (main) ONLY — per-model GPU tuning for the prod environment (1x H200)
...
`Dev` branch has the equivalent `dev/gemma-4-31b-it.env`, `dev/medgemma-1.5-4b-it.env`, `dev/medgemma-27b-it.env` instead of `main/`. `Qa` branch has `qa/` with the same three files, tuned for its L40S GPUs.
